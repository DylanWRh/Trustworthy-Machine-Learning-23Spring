...
parser.add_argument('--anp-eps', type=float, default=0.2)
...
def mask_train(model, criterion, mask_opt, noise_opt, data_loader):
    ...
    for i, (images, labels) in enumerate(data_loader):
        ...
        for _ in range(args.anp_steps):
            noise_opt.zero_grad()
            include_noise(model)
            logits_with_noise = model(images)
            loss_with_noise = -criterion(logits_with_noise, labels)
            loss_with_noise.backward()
            sign_grad(model)
            noise_opt.step()
        ...
        include_noise(model)
        logits_with_noise = model(images)
        loss_rob = criterion(logits_with_noise, labels)
        
        exclude_noise(model)
        logits = model(images)
        loss_nat = criterion(logits, labels)
        ...
        # update total_correct by adding correct predictions made by masked models without noise
        total_correct += len(torch.where(logits.max(dim=1)[1] == labels)[0])
